...
### The [IBinarySerializer](Serialization/IBinarySerializer.cs)
...
[BinarySerializerContext](Serialization/BinarySerializerContext.cs)
and [BinaryDeserializerContext](Deserialization/BinaryDeserializerContext.cs) are simple
non concurrent caches that can be reused to avoid recomputing Type to [ISerializationDriver](Serialization/Abstractions/ISerializationDriver.cs)
...
These contexts are only caches: mappings definition are managed by the thread safe [SharedBinarySerializerContext](Serialization/SharedBinarySerializerContext.cs)
and [SharedBinaryDeserializerContext](Deserialization/SharedBinaryDeserializerContext.cs) that use Resolvers
([ISerializerResolver](Serialization/Abstractions/ISerializerResolver.cs)
...
[SharedSerializerKnownObject](Serialization/Resolvers/SharedSerializerKnownObject.cs) and
...
They can be rather simple ([BasicTypesSerializerResolver](Serialization/Resolvers/BasicTypesSerializerResolver.cs))
...
classDiagram
    ...
    class IBinarySerializer {
        <<interface>>
        +ICKBinaryWriter Writer
        +BinarySerializerContext Context
        +bool IsDebugMode
        +WriteAny(object o) bool
        +WriteAnyNullable(object? o) bool
        +WriteTypeInfo(Type t, bool? nullable) bool
        +WriteObject~T~(T o) bool
        +WriteNullableObject~T~(T? o) bool
        +WriteValue~T~(in T value)
        +WriteNullableValue~T~(in T? value)
        +DebugWriteMode(bool? active)
        +DebugWriteSentinel(string? fileName, int line)
        +event OnDestroyedObject
    }
    class ICKBinaryWriter {
        <<interface>>
        +Stream BaseStream
        +CKBinaryWriter.ObjectPool~string~ StringPool
        +Flush()
        +Write(...) 25 overloads
        +WriteNullableInt32(int? i) and 21 more named variants
    }
    class BinarySerializerContext {
        +SharedBinarySerializerContext Shared
        +SimpleServiceContainer Services
        +int MaxRecursionDepth
        +Statistics LastStatistics
        +FindDriver(Type t) ISerializationDriver
        +TryFindDriver(Type t) ISerializationDriver?
        +TryFindPossiblyAbstractDriver(Type t) ISerializationDriver?
        +IsSerializable(Type t) bool
        +GetKnownObjectKey(object o) string?
    }
    class Statistics {
        <<struct>>
        +int DriverLookup
        +int DriverSharedLookup
        +int DriverSharedContextCached
        +int DriverContextCached
        +int DriverNeverCached
        +int DriverNonSealed
    }
    class SharedBinarySerializerContext {
        +ISerializerKnownObject KnownObjects
        +AddResolver(ISerializerResolver resolver, bool beforeExisting)
        +AddSerializationDriver(Type t, ISerializationDriver driver)
        +SetNotSerializable(Type t)
        +TryFindDriver(BinarySerializerContext context, Type t) ISerializationDriver?
        +GetStaticWriter(Type writerHost, Type instanceType)$ MethodInfo
    }
    class IDestroyable {
        <<interface>>
        +bool IsDestroyed
    ...
    IBinarySerializer o-- ICKBinaryWriter : Writer
    IBinarySerializer o-- BinarySerializerContext : Context
    BinarySerializerContext *-- Statistics : LastStatistics
    BinarySerializerContext o-- SharedBinarySerializerContext : Shared
    SharedBinarySerializerContext ..|> ISerializerResolver
    IBinarySerializer ..> IDestroyable : optional, reference types only
```
...
See [here](https://github.com/signature-opensource/CK-Core/tree/dev/stable/CK.Core/SimpleSerialization).
...
The code above shows the hook registration. The hook has access to a [IMutableTypeReadInfo](Deserialization/TypeReadInfo/IMutableTypeReadInfo.cs).

```mermaid
classDiagram
    direction LR
    class IMutableTypeReadInfo {
        <<interface>>
        +IWrittenInfo WrittenInfo
        +IDeserializationDriver? CurrentDriver
        +string? CurentDriverName
        +Type? CurrentTargetType
        +string CurrentAssemblyName
        +string CurrentTypeNamespace
        +string CurrentTypeName
        +SetDriver(IDeserializationDriver driver)
        +SetDriverName(string driverName)
        +SetTargetType(Type t)
        +SetLocalTypeAssemblyName(string assemblyName)
        +SetLocalTypeNamespace(string typeNamespace)
        +SetLocalTypeName(string typeName)
    }
    class IWrittenInfo {
        <<interface>>
        +TypeReadInfoKind Kind
        +bool IsValueType
        +bool IsSealed
        +int ArrayRank
        +ITypeReadInfo? BaseTypeReadInfo
        +IReadOnlyList~ITypeReadInfo~ SubTypes
        +string? DriverName
        +int Version
        +string AssemblyName
    ...
    class ITypeReadInfo {
        ...
        +ITypeReadInfo? BaseTypeReadInfo
        +IReadOnlyList~ITypeReadInfo~ SubTypes
        +IReadOnlyList~ITypeReadInfo~ TypePath
        +bool HasResolvedConcreteDriver
        +TryResolveLocalType() Type?
        +ResolveLocalType() Type
        +GetConcreteDriver(Type? expected) IDeserializationDriver
        +GetPotentiallyAbstractDriver(Type? expected) IDeserializationDriver
    }
    IMutableTypeReadInfo o-- IWrittenInfo : what was written
    IMutableTypeReadInfo ..> ITypeReadInfo : hooks feed the final
```
...
base classes for reference type ([`ReferenceTypeDeserializer<T>`](Deserialization/Implementation/ReferenceTypeDeserializer.cs) 
and [`SimpleReferenceTypeDeserializer<T>`](Deserialization/Implementation/SimpleReferenceTypeDeserializer.cs)) directly 
...
[`ValueTypeDeserializer<T>`](Deserialization/Implementation/ValueTypeDeserializer.cs) is not able to handle
this mutation. When a serialized stream that has been written with classes must be read back, 
the [`ValueTypeDeserializerWithRef<T>`](Deserialization/Implementation/ValueTypeDeserializerWithRef.cs)
must be used.
...
- `CK.Core` for `ICKBinaryWriter` / `ICKBinaryReader` and the two simple serialization contracts, and
  `CSemVer`.
